  ...
  const renderVocab = () => (
    <div className="animate-fade-in flex flex-col items-center justify-center h-full w-full max-w-lg mx-auto py-2">
      ...
      <div className="w-full flex justify-between items-end mb-2 shrink-0">
        <h2 className="text-lg font-bold text-gray-800">{vocabMode === 'flashcard' ? 'Luyện Phản Xạ' : `Test Trí Nhớ (Điểm: ${quizScore})`}</h2>
      ...
      {vocabMode === 'flashcard' ? (
        <div className="w-full flex-1 min-h-[300px] cursor-pointer" style={{ perspective: '1000px' }} onClick={() => setIsFlipped(!isFlipped)}>
          <div className="w-full h-full relative transition-transform duration-500" style={{ transformStyle: 'preserve-3d', transform: isFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
            <div className="absolute inset-0 bg-white border border-gray-200 rounded-3xl flex flex-col items-center justify-center p-6 hover:border-blue-300 shadow-md transition-colors" style={{ backfaceVisibility: 'hidden' }}>
              <span className="text-xs font-bold uppercase text-gray-500 mb-3 bg-gray-100 px-3 py-1 rounded-full">{allVocab[cardIdx].cat}</span>
              <h3 className="text-3xl sm:text-4xl font-black text-gray-900 mb-2 text-center tracking-tight">{allVocab[cardIdx].word}</h3>
              <p className="text-base text-blue-500 font-mono mb-4">{allVocab[cardIdx].ipa}</p>
              <div className="absolute bottom-4 w-full px-6 flex justify-between items-center" onClick={(e) => e.stopPropagation()}>
                <button onClick={() => setShowHint(!showHint)} className="text-xs font-medium text-amber-600 bg-amber-50 px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-amber-100 transition border border-amber-100">
                  <IconLightbulb /> {showHint ? "Ẩn gợi ý" : "Xem gợi ý"}
                </button>
                <button onClick={() => playAudio(allVocab[cardIdx].word)} className="p-2.5 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-full transition-colors shadow-sm"><IconPlay /></button>
              </div>
              {showHint && (
                <div className="absolute bottom-16 w-full px-6 text-center animate-fade-in" onClick={(e) => e.stopPropagation()}>
                  <p className="text-xs text-gray-600 bg-amber-50/50 p-2.5 rounded-xl border border-amber-100 italic font-medium">{allVocab[cardIdx].hint}</p>
                </div>
              )}
            </div>
            <div className="absolute inset-0 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl flex flex-col items-center justify-center p-6 text-center shadow-lg" style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
              <h3 className="text-2xl font-bold text-white mb-4">{allVocab[cardIdx].vi}</h3>
              <div className="bg-white/10 backdrop-blur-sm p-4 rounded-2xl w-full text-left border border-white/20">
                <p className="text-blue-200 text-[10px] font-bold uppercase mb-1 tracking-wider">Ví dụ</p>
                <p className="text-white font-medium text-base sm:text-lg leading-relaxed">"{allVocab[cardIdx].ex}"</p>
              </div>
            </div>
          </div>
        </div>
      ) : (
        <div className="w-full flex-1 flex flex-col bg-white border border-gray-200 rounded-3xl p-5 shadow-sm text-center overflow-y-auto">
          <p className="text-gray-500 text-xs mb-2 font-medium">Chọn từ tiếng Anh có nghĩa là:</p>
          <h3 className="text-2xl font-bold text-gray-900 mb-4 shrink-0">"{allVocab[cardIdx].vi}"</h3>
          <div className="grid grid-cols-1 gap-2 shrink-0">
            {quizOptions.map((opt, i) => {
              ...
              return (
                <button key={i} disabled={quizAnswered !== null} onClick={() => handleQuizAnswer(opt)} className={`py-3 rounded-xl transition-all border shadow-sm flex items-center justify-between px-5 text-sm ${btnStyle}`}>
          ...
          {quizAnswered && (
            <div className="mt-4 text-left animate-fade-in bg-blue-50 border border-blue-100 rounded-2xl p-4 shadow-sm relative overflow-hidden shrink-0">
              <div className="absolute top-0 right-0 p-3 opacity-10 text-blue-500 transform scale-[2]"><IconSparkles /></div>
              ...
              <p className="text-gray-700 text-sm mb-1">
                ...
                Từ đúng là <strong className="text-blue-700 text-base">{allVocab[cardIdx].word}</strong>
                <span className="text-gray-500 ml-2 font-mono bg-white px-1.5 py-0.5 rounded border border-gray-200 text-xs">{allVocab[cardIdx].ipa}</span>
                {' '}— <span className="text-gray-600">{allVocab[cardIdx].vi}</span>
              </p>
              <div className="bg-white p-2.5 rounded-lg border border-blue-100 flex justify-between items-center mt-2">
              ...
              <button onClick={() => setCardIdx((prev) => (prev + 1) % allVocab.length)} className="mt-3 w-full py-2.5 bg-blue-600 hover:bg-blue-700 rounded-xl text-white text-sm font-bold transition-colors shadow-md shadow-blue-500/30">
                Làm câu tiếp theo
              </button>
            </div>
          )}
      ...
      {vocabMode === 'flashcard' && (
        <div className="flex justify-center items-center gap-4 mt-4 shrink-0 w-full">
          <button onClick={() => { setIsFlipped(false); setShowHint(false); setTimeout(() => setCardIdx((prev) => (prev - 1 + allVocab.length) % allVocab.length), 150); }} className="flex-1 py-2.5 bg-white border border-gray-200 hover:bg-gray-50 rounded-xl text-gray-700 font-bold text-sm transition-colors shadow-sm">Trước</button>
          <button onClick={() => { setIsFlipped(false); setShowHint(false); setTimeout(() => setCardIdx((prev) => (prev + 1) % allVocab.length), 150); }} className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 rounded-xl text-white font-bold text-sm transition-colors shadow-md shadow-blue-500/30">Tiếp theo</button>
        </div>
      )}
    </div>
  );
    ...
    <div className="animate-fade-in flex flex-col h-full w-full max-w-3xl mx-auto py-2">
      ...
      {listenTabMode === 'dictation' && (
        <>
          ...
          {showListenAnswer && (
            <div className="bg-green-50 border border-green-200 rounded-3xl p-5 mb-4 animate-fade-in shadow-sm relative shrink-0">
              ...
              <div className="bg-white rounded-2xl p-4 border border-green-100 shadow-sm">
                <div className="flex items-center gap-2 mb-2 border-b border-green-50 pb-2">
                  <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center shadow-sm border border-green-200">
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-600 to-teal-600 font-black text-xs">AI</span>
                  </div>
      ...
      {listenTabMode === 'running' && (
        <div className="flex flex-col gap-4 flex-1 overflow-y-auto">
          <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-2xl px-5 py-4 shrink-0">
            <div className="text-center">
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold mb-0.5">Thời gian</p>
              <p className="text-3xl font-mono font-black text-gray-800">{formatTime(runSeconds)}</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className={`text-sm font-bold ${runFinished ? 'text-blue-600' : runPlaying ? 'text-green-600' : 'text-gray-400'}`}>
    ...
    <div className="animate-fade-in flex flex-col h-full w-full max-w-4xl mx-auto py-2">
      ...
      <div className="flex-1 bg-white rounded-3xl p-5 border border-gray-200 shadow-sm flex flex-col min-h-0">
        ...
        <textarea
          ...
          className={`w-full flex-1 bg-gray-50 border p-4 rounded-2xl text-gray-900 text-base resize-none outline-none transition-colors ${isRecording ? 'border-red-300 ring-2 ring-red-100' : 'border-gray-200 focus:border-rose-400'}`}
    ...
    <div className="animate-fade-in flex flex-col h-full w-full max-w-4xl mx-auto py-2">
      ...
      <div className="flex-1 flex flex-col bg-white border border-gray-200 rounded-3xl shadow-sm overflow-hidden min-h-0">
        ...
        <textarea
          ...
          disabled={isGrading}
          className="flex-1 w-full bg-transparent p-5 text-gray-900 text-base resize-none outline-none"
